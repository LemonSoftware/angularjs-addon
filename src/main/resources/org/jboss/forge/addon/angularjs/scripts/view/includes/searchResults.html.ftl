            ...
            <#if (property.hidden!"false") != "true">
                <#if (property["many-to-one"]!"false") == "true" || (property["one-to-one"]!"false") == "true">
                    <#if (property["option-label-temporal-type"]!"") == "date">
                        <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}
                            .${property.optionLabel}| date:'mediumDate'}}</a></td>
                    <#elseif (property["option-label-temporal-type"]!"") == "time">
                        <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}
                            .${property.optionLabel}| date:'mediumTime'}}</a></td>
                    <#elseif (property["option-label-temporal-type"]!"") == "both">
                        <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}
                            .${property.optionLabel}| date:'medium'}}</a></td>
                    <#else>
                        <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}
                            .${property.optionLabel}}}</a></td>
                    </#if>
                ...
                <#elseif (property["datetime-type"]!"") == "time">
                    <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}|
                        date:'mediumTime'}}</a></td>
                <#elseif (property["datetime-type"]!"") == "both">
                    <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}|
                        date:'yyyy-MM-dd HH:mm:ss Z'}}</a></td>
                <#else>
                    <td><a href="#/${pluralizedEntityName}/edit/{{result.${entityId}}}">{{result.${property.name}}}</a>
                    </td>
                </#if>
            </#if>
        </#list>
